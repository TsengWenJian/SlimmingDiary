...
class MakeDiarysTableViewController: UITableViewController {
    ...
    @objc func uploadTitleImage() {
        ...
        if actionType == .update {
            uploadDiaryWithItemsImage(diarys: diarys)

            return
        }

        guard let myTitleImage = titleImage?.resizeImage(maxLength: 1024),
              let dataImage = UIImageJPEGRepresentation(myTitleImage, 0.8)
        else {
            return
        }
        ...
        serviceManager.storageTitleImagesURL.child("image_\(imageUid)_\(myTitleImage.hash).jpg").putData(dataImage, metadata: nil) { metadata, error in
            ...
            ref.updateChildValues(dict) { error, _ in
                ...
                self.serviceManager.dbUserDiaryURL.child(userId).child(self.diaryId).updateChildValues(dict, withCompletionBlock: { error, _ in
    ...
    func uploadImageIntoStorage(items: [DiaryItem]) {
        ...
        for item in items {
            guard let myImage = item.image,
                  let data = UIImageJPEGRepresentation(myImage, 0.8)
            else {
    ...
    func uploadDairyToDB(id: String) {
        ...
        if actionType == .insert {
            serviceManager.dbDiaryContentURL.child(id).updateChildValues(["diaryRecords": records]) { error, _ in
            ...
            let originOpen = trackIsLock == true ?"private" : "public"
            let currnetOpen = isLocked == true ?"private" : "public"
               ...
               let uid = serviceManager.userUid
            {
                ...
                serviceManager.dbDiarysURL.child(currnetOpen).child(myDiaryId).setValue(dict)
                serviceManager.dbDiarysURL.child(originOpen).child(myDiaryId).removeValue()
                serviceManager.dbUserDiaryURL.child(uid).child(myDiaryId).removeValue()
                serviceManager.dbUserDiaryURL.child(uid).child(myDiaryId).setValue(dict)
            }

            serviceManager.dbDiaryContentURL.child(titleDiary.diaryId).updateChildValues(["diaryRecords": records], withCompletionBlock: { _, _ in
                self.toastView.removefromView()
